...
| [upsilonauth and upsiloneconomy unit tests are never executed by CI or local CI](issues/ISS-132_20260826_auth_economy_unit_tests_not_run.md) | 2026-08-26 | Open | High | The `go vet` and `go test` module lists in `.github/workflows/ci.yml` and `sc... |
| [Battle lifecycle SSE events never arrive after PvE match.found (60s dead air)](issues/ISS-131_20260819_battle_sse_events_never_arrive.md) | 2026-08-19 | Open | High | `e2e_skill_equip_battle` joins a `1v1_PVE` match, receives `match.found` and ... |
| [Revoked/logged-out bearer token is not rejected at the auth layer](issues/ISS-130_20260819_revoked_token_not_rejected.md) | 2026-08-19 | Open | High | `edge_auth_session_timeout` revokes a session token via `auth_logout`, then r... |
