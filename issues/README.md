...
| ISS-132 | [ISS-132_20260826_auth_economy_unit_tests_not_run.md](ISS-132_20260826_auth_economy_unit_tests_not_run.md) | High | Open | `go vet`/`go test` module lists in `ci.yml`, `run_ci_local.sh`, and `run_all_unit_tests.sh` were never updated after the auth/economy extraction — 17 real `_test.go` files under `upsilonauth`/`upsiloneconomy` are never executed by any test runner, only exercised indirectly via the docker-compose integration stage |
| ISS-131 | [ISS-131_20260819_battle_sse_events_never_arrive.md](ISS-131_20260819_battle_sse_events_never_arrive.md) | High | Open | After PvE `match.found`, no `board.updated`/`turn.started`/`game.started`/`game.ended` SSE event arrives within 60s — total dead air, not the known fast forfeit-race window; first seen in the 2026-08-19 CI run (first E2E run since Phase 4/5 auth/economy extraction was pushed) |
| ISS-130 | [ISS-130_20260819_revoked_token_not_rejected.md](ISS-130_20260819_revoked_token_not_rejected.md) | High | Open | A revoked/logged-out bearer token is not rejected — reused token gets 404 "not enrolled in battle" instead of 401, meaning the request reached a game handler past auth; possible security boundary bypass; first seen in the 2026-08-19 CI run |
